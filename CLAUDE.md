...
Asset folders/files that exist but aren't wired into any code yet: most of `assets/images/effects/` (`laser-flash`, `big-explosion`, `big-fragments`, `small-fragments`, `smoke`, `bullet-impacts` — see GDD.md's Effects & projectiles section for what each would need), `assets/images/projectiles/Grenade.png`/`RPG-round.png` (no AoE weapon exists yet), `assets/images/obstacles_and_objects/` (no visible/decorative obstacles yet, only the invisible collision walls from the Tiled map's `"wall"` objects), 2 of the 6 soldier classes in `assets/images/soliders/` (Grenadier, RadioOperator — blocked on mechanics, not just stats), and Centipede in `assets/images/robots/` (blocked on designing a segmented-body rendering approach). See GDD.md's content-inventory table.
