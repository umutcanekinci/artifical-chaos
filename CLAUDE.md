...
- **Drone / Scarab / Spider** (`gameplay/robot.py`): `Drone` is the shared base class, taking a `drone_type` string that looks up `hp`/`speed`/`melee_range`/`fire_range`/`melee_damage`/`fire_damage`/`melee_cooldown_ms`/`fire_cooldown_ms` from `DRONE_TYPES` (`util/constants.py`) and picks the matching sprite sheet; `Scarab`/`Spider` are thin subclasses that just pin `drone_type`, and `DRONE_CLASSES = {"Scarab": Scarab, "Spider": Spider}` is the spawn-time lookup `map.py` uses. Both run the same combat state machine each frame (`engage()`): idle until the player or an in-army soldier is within `AGGRO_RADIUS` (shared across types; `get_target()`, nearest-wins via `combat.find_nearest` — will chase a closer soldier over a farther player), then approaches; within `self.fire_range` it fires, within the tighter `self.melee_range` it melees instead, each gated by its own cooldown. Spider is tuned as a melee-preferring flanker (higher speed, much shorter `fire_range` than Scarab so it rarely lingers at range). `die()` sets `status = "destroyed"` and starts a timer; `update()` holds the destroyed pose for `DESTROYED_DURATION_MS` (shared across types) before setting `self.active = False`, which `Game._purge_inactive()` then removes from `game.robots` on the next frame. Hornet/Wasp/Centipede aren't wired up — their sheet layouts don't match the idle/walk/fire/melee/destroyed row pattern Scarab/Spider share (see GDD.md).
...
Asset folders that exist but aren't wired into any code yet: `assets/Effects/` (explosions, muzzle flashes, hit sparks/spatters, smoke), `assets/Projectiles/` (bullets+plasma, grenade, RPG round), 2 of the 6 soldier classes in `assets/images/soliders/` (Grenadier, RadioOperator — blocked on mechanics, not just stats), 3 of the 5 drone types in `assets/images/robots/` (Hornet, Wasp, Centipede — blocked on confirming sheet layout). See GDD.md's content-inventory table.
